...
public class PlayerSlider implements GameInstance {
    ...
    public void draw(float x, float y, int mouseX, int mouseY) {
        ...
        float sliderX = x + 15f;
        double currentTime = Reversal.musicManager.screen.player.getCurrentTime();
        double totalTime = Reversal.musicManager.screen.player.getMusic().getDuration();
    ...
    public void mouseReleased() {
        if (dragging) {
            dragging = false;
            long duration = Reversal.musicManager.screen.player.getMusic().getDuration();
            double newTime = duration * (dragDelta / 170f);
            if (newTime < 0) newTime = 0;
            if (newTime > duration) newTime = duration;
            Reversal.musicManager.screen.player.setCurrentTime(newTime);
        }
    }
