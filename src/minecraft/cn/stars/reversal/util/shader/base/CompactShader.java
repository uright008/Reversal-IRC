...
public abstract class CompactShader {

    private int program;

    private Map<String, Integer> uniformsMap;

    public CompactShader(final String fragmentShader) {
        int vertexShaderID, fragmentShaderID;

        try {
            final InputStream vertexStream = getClass().getResourceAsStream("/assets/minecraft/reversal/shader/vertex.vsh");
            vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB);
            IOUtils.closeQuietly(vertexStream);

            final InputStream fragmentStream = getClass().getResourceAsStream("/assets/minecraft/reversal/shader/main_menu/" + fragmentShader);
