...
public final class ClientName extends Command {
    ...
    public void onCommand(final String command, final String[] args) throws Exception {
        if (args[0].equals("reset")) {
            ...
            StarX.INSTANCE.getNotificationManager().registerNotification("Successfully reset the client name.", NotificationType.SUCCESS);
            StarX.INSTANCE.showMsg("Successfully reset the client name.");
        } else {
            ThemeUtil.setCustomClientName(String.join(" ", args));
            StarX.INSTANCE.getNotificationManager().registerNotification("Successfully set the client name.", NotificationType.SUCCESS);
