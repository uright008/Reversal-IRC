...
class WingUtils : ModelBase() {
    ...
    private val location: ResourceLocation = ResourceLocation("starx/images/wings.png")
    ...
    fun renderWings(partialTicks: Float) {
        ...
        val rotate = interpolate(
            if (per) mc.thePlayer.rotationYaw else mc.thePlayer.prevRenderYawOffset,
            if (per) mc.thePlayer.rotationYaw else mc.thePlayer.renderYawOffset,
            partialTicks
        )
        GL11.glPushMatrix()
